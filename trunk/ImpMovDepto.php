...
document.location=('ImpMovDepto.php?escolha=' + escolha);
}
}
...
case '01': // imprime relatorio de movimentacoes financeiras de departamento por Beneficiado em html
	...
	$sql="SELECT * FROM movimentacao, funcdepto,categoria,associadepto WHERE beneficiado=cod_funcdepto AND movimentacao.categoria=categoria.cod_categoria AND departamento=deptosuperior ORDER BY departamento"; 
	...
	echo "<th width=300>Departamento</th>"; // th -> coluna
	
	echo "<th width=200>Dpto Subordinado</th>";
    echo "<th width=50>Categoria</th>";
    echo "<th width=200>Valor </th>";
	...
	while ($linha=mysql_fetch_array($resultado)){
		...
		$campo0=$linha["departamento"]; 
				
		$campo1=$linha["deptosubordinado"]; 
...
echo "<form action='relatoriopdf.php' method='post'>";
	...
	while ($linha=mysql_fetch_array($resultado)){
	   ...
	   echo "</table>";
		...
		echo "<input type='hidden' name='opcao' value='14' >";//envia valor para relatoriopdf.php 
