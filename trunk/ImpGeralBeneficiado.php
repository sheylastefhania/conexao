...
document.location=('ImpGeralBeneficiado.php?escolha=' + escolha);
}
}
...
case '01': // imprime relatorio de movimentacoes financeiras de departamento por Beneficiado em html
	...
	$sql="SELECT * FROM movimentacao, funcdepto,categoria WHERE beneficiado=cod_funcdepto AND movimentacao.categoria=categoria.cod_categoria ORDER BY beneficiado,data_movimentacao"; 
...
echo "<form action='relatoriopdf.php' method='post'>";
	...
	while ($linha=mysql_fetch_array($resultado)){
	   ...
	   echo "</table>";
		...
		echo "<input type='hidden' name='opcao' value='12' >";//envia valor para relatoriopdf.php 
