...
document.location=('ImpMovEstado.php?escolha=' + escolha);
}
}
...
case '01': // relatorio de movimentacao por estado de operacao(Confirmada/Previsao/Todas) em html

	echo "<form action='exibir.php' method='post'>";


// SELECT PARA TIPO DE OPERACAO
	
	echo "Deixe em branco para ver todos:  ";
	echo "<select name='veroperacao' >";  // valor onde sera armazenado o valor escolhido
	
	$sql=mysql_query("SELECT DISTINCT estado_operacao FROM movimentacao") or die ("Houve erro na selecao da tabela");
	...
	while ($linha=mysql_fetch_array($sql)) {
		...
		$campoop=$linha['estado_operacao'];
		
	    echo "<option value='$campoop'>$campoop</option>"; // 
	}						   
        echo "</select>";
       ...
       echo "<input type='hidden' name='opcao' value='11' >";	//opcao enviada a exibir.php 
...
echo "<form action='relatoriopdf.php' method='post'>";
	...
	while ($linha=mysql_fetch_array($sql)) {
       ...
       //echo "<input type='hidden' name='codigo' value='$codigo' >";	//opcao enviada a exibir.php 
       echo "<input type='hidden' name='opcao' value='11' >";	//opcao enviada a relatoriopdf.php 
       echo "<input name='submit' type='submit' value='Visualizar'/>";
	   echo "</form>";	
